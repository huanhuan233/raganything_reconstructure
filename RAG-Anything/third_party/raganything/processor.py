...
from third_party.raganything.base import DocStatus
from third_party.raganything.parser import MineruParser, MineruExecutionError, get_parser
from third_party.raganything.utils import (
    separate_content,
    insert_text_content,
...
class ProcessorMixin:
    ...
    async def parse_document(
        ...
        **kwargs,
        ...
        # MinerU 3.x：默认后端常为 hybrid-auto-engine（更重）；未传 -b 时走默认。
        # 设备由子进程读 MINERU_DEVICE_MODE / torch.cuda 决定，CLI 的 -d 会被忽略。
        if self.config.parser == "mineru":
            kwargs = dict(kwargs)
            backend_env = os.getenv("MINERU_BACKEND", "").strip()
            if backend_env and kwargs.get("backend") is None:
                kwargs["backend"] = backend_env
                self.logger.info(
                    "MinerU backend from MINERU_BACKEND env: %s", backend_env
                )
    ...
    def _apply_chunk_template(
        ...
        """
        from third_party.raganything.prompt import PROMPTS
