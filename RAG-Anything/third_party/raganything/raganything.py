...
from third_party.raganything.query import QueryMixin
from third_party.raganything.processor import ProcessorMixin
from third_party.raganything.batch import BatchMixin
from third_party.raganything.utils import get_processor_supports
from third_party.raganything.parser import MineruParser, SUPPORTED_PARSERS, get_parser

# Import specialized processors
from third_party.raganything.modalprocessors import (
    ImageModalProcessor,
    TableModalProcessor,
...
class RAGAnything(QueryMixin, ProcessorMixin, BatchMixin):
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    # Neo4j 等异步驱动绑定在主循环上；主 asyncio.run 已结束时
                    # 再收尾会报 Event loop is closed。应在业务代码里 await finalize_storages。
    ...
    async def _ensure_lightrag_initialized(self):
        ...
        try:
            ...
            # Allow storage backend override from environment for easier deployment.
            env_storage_mapping = {
                "kv_storage": os.getenv("KV_STORAGE")
                or os.getenv("LIGHTRAG_KV_STORAGE"),
            ...
            for key, value in env_storage_mapping.items():
                if value and key not in self.lightrag_kwargs:
