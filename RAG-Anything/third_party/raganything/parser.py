    ...
    def parse_pdf(
        ...
        **kwargs,
        ...
        try:
            ...
            self.logger.info(f"[MinerU Parser] Total pages: {total_pages}")
            batch_size = 2
            ...
            for start in range(0, total_pages, batch_size):
                ...
                self.logger.info(
                    f"[MinerU Parser] Processing pages {start + 1}-{end + 1}/{total_pages}"
                )
                page_output_dir = base_output_dir / f"pages_{start}_{end}"
                page_output_dir.mkdir(parents=True, exist_ok=True)
                self.logger.info(f"[MinerU Parser] Output dir: {page_output_dir}")
                ...
                content_list, _ = self._read_output_files(
                    page_output_dir, name_without_suff, method=method
                )
                self.logger.info(
                    f"[MinerU Parser] Pages {start + 1}-{end + 1} extracted {len(content_list)} content blocks"
                )
                all_content.extend(content_list)

            self.logger.info(
                f"[MinerU Parser] Total extracted content blocks: {len(all_content)}"
            )
            if failed_pages:
                self.logger.warning(f"[MinerU Parser] Failed pages: {failed_pages}")
            return all_content
...
class DeepseekOCR2Parser(Parser):
    """DeepSeek OCR2 parser based on HuggingFace Transformers inference."""

    def __init__(self, default_model: str = "deepseek-ai/DeepSeek-OCR-2") -> None:
        super().__init__()
    ...
    def _deepseek_use_subprocess(self) -> bool:
        """When ``DEEPSEEK_OCR2_PYTHON`` points to another env's interpreter, run OCR there."""
        sub = os.environ.get("DEEPSEEK_OCR2_PYTHON", "").strip()
        if not sub:
            return False
        if os.environ.get("RAGANYTHING_DEEPSEEK_WORKER") == "1":
            return False
        return True

    def _run_deepseek_subprocess(
        self,
        file_path: Path,
        mode: str,
        **kwargs: Any,
    ) -> List[Dict[str, Any]]:
        """Run ``python -m raganything.deepseek_ocr2_worker`` in the OCR2 conda environment."""
        del kwargs  # reserved; extend worker CLI if needed
        python_exe = os.environ["DEEPSEEK_OCR2_PYTHON"].strip()
        ...
        try:
            ...
            env["RAGANYTHING_DEEPSEEK_WORKER"] = "1"
            env.pop("DEEPSEEK_OCR2_PYTHON", None)
            cmd = [
                python_exe,
                "-m",
                "raganything.deepseek_ocr2_worker",
                "--out-json",
                str(out_path),
            ]
            if mode == "pdf":
                ...
                raise ValueError(f"Unsupported subprocess mode: {mode}")
            timeout = int(os.environ.get("DEEPSEEK_OCR2_SUBPROCESS_TIMEOUT", "7200"))
            proc = subprocess.run(
                cmd,
                cwd=str(root),
                env=env,
            ...
            if proc.returncode != 0:
                ...
                raise MineruExecutionError(
                    proc.returncode,
                    tail[-30:] if tail else ["(no subprocess output)"],
                )
            raw = out_path.read_text(encoding="utf-8")
            data = json.loads(raw)
            if not isinstance(data, list):
                raise ValueError("deepseek_ocr2_worker returned non-list JSON")
            return data
        finally:
            try:
    ...
    def _require_transformers(self):
        try:
            ...
            raise ImportError(
                "DeepSeek OCR2 parser requires `transformers` and `torch`. "
                "Install dependencies in your DeepSeek OCR2 environment first."
            ) from exc
        return torch, AutoModel, AutoTokenizer

    def _get_model_components(self, model_name: Optional[str] = None):
        ...
        torch, AutoModel, AutoTokenizer = self._require_transformers()
        model_name = (model_name or self.default_model).strip() or self.default_model
    ...
    @staticmethod
    def _to_text_blocks(markdown_text: str, page_idx: int) -> List[Dict[str, Any]]:
        if not markdown_text:
            return []
        return [
            {"type": "text", "text": markdown_text.strip(), "page_idx": int(page_idx)}
        ]
    ...
    def _infer_image_markdown(
        ...
        _, tokenizer, model = self._get_model_components(model_name=model_name)
        prompt = (prompt or "<image>\n<|grounding|>Convert the document to markdown.").strip()
        ...
        with tempfile.TemporaryDirectory() as output_dir:
            try:
                ...
                # Fallback for older/newer infer signatures.
                result = model.infer(
                    tokenizer,
                    prompt=prompt,
                    image_file=str(image_path),
    ...
    def _extract_pdf_page_inputs(self, pdf_path: Path) -> Iterator[Tuple[int, Any]]:
        try:
            ...
            raise ImportError(
                "PDF parsing with parser='deepseek_ocr2' requires `pypdfium2`. "
    ...
    def parse_pdf(
        ...
        **kwargs,
        ...
        del output_dir, method, lang
        pdf_path = Path(pdf_path)
        if not pdf_path.exists():
            raise FileNotFoundError(f"PDF file does not exist: {pdf_path}")

        if self._deepseek_use_subprocess():
            return self._run_deepseek_subprocess(pdf_path, "pdf", **kwargs)

        prompt = kwargs.get("prompt")
        model_name = kwargs.get("model_name")
        base_size = int(kwargs.get("base_size", 1024))
        image_size = int(kwargs.get("image_size", 768))
        crop_mode = bool(kwargs.get("crop_mode", True))
            ...
            for page_idx, rendered_page in page_inputs:
                ...
                try:
                    ...
                    if hasattr(rendered_page, "save"):
                        ...
                        Image.fromarray(rendered_page).save(temp_image_path)
                    markdown_text = self._infer_image_markdown(
                    ...
                    content_list.extend(
                        self._to_text_blocks(markdown_text=markdown_text, page_idx=page_idx)
                    )
                finally:
                    if temp_image_path is not None and temp_image_path.exists():
                        try:
    ...
    def parse_image(
        ...
        **kwargs,
        ...
        if self._deepseek_use_subprocess():
            return self._run_deepseek_subprocess(image_path, "image", **kwargs)
        ...
        return self._to_text_blocks(markdown_text=markdown_text, page_idx=page_idx)
    ...
    def parse_document(
        ...
        **kwargs,
        ...
        raise ValueError(
            f"Unsupported file format: {ext}. "
            "DeepSeek OCR2 parser supports PDF, image, office, and text formats."
        )

    def check_installation(self) -> bool:
        if self._deepseek_use_subprocess():
            exe = os.environ.get("DEEPSEEK_OCR2_PYTHON", "").strip()
            ...
            try:
                proc = subprocess.run(
                    [
                        exe,
                        "-c",
                        "import torch, transformers; import pypdfium2 as _p; print('ok')",
                    ],
                    capture_output=True,
                    text=True,
                    timeout=120,
...
SUPPORTED_PARSERS = ("mineru", "docling", "paddleocr", "deepseek_ocr2")
...
def get_parser(parser_type: str) -> Parser:
    ...
    if parser_name == "deepseek_ocr2":
        return DeepseekOCR2Parser()
    raise ValueError(
